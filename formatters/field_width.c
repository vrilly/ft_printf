...
/*   Updated: 2019/12/05 18:08:42 by tjans         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	set_neg(char *str, t_flags *flags, int s_len)
{
...
{
	char		*newstr;
	const char	pad_c =(flags->pad_zeroes && !flags->precision &&
			!flags->pad_neg_f_width) ? '0' : ' ';
	...
	if (flags->pad_neg_f_width)
		...
		ft_memcpy(newstr +
				(flags->min_field_width_n - s_len) + is_neg, str + is_neg, s_len);
	if (is_neg)
